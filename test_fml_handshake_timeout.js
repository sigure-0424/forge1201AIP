...
bot.on('inject_allowed', () => {
    bot._client.on('packet', (data, meta) => {
        if (meta.name === 'login_plugin_request' && data.channel === 'fml:loginwrapper') {
            ...
            if (disc === 5) {
                // Let's try sending discriminator 1 (ClientHello) first?
                console.log("[Forge] Replying with ClientHello (Disc 1, FML ver 3) then Client ModList (Disc 2?)");
                // The server sent discriminator 5. We must reply with our state.
                replyPayload = Buffer.from([1, 3]); // Maybe this isn't right if disc is 5.
                
                // Let's try sending discriminator 5 back but with an empty mod list? Wait, I tried [5, 0] before.
...
setTimeout(() => {
    console.log('Timeout. Exiting to avoid hanging.');
    process.exit(0);
}, 5000);
